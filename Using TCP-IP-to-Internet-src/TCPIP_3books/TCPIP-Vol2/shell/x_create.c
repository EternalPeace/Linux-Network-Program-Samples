...
{
#ifndef FIXME
    fprintf(stderr,"create: create will be rewritten when a future release\n");
	...
	for (found=FALSE ; !found ;) {
		...
		for (symptr=(struct nlist *)buf ; symptr<last ;symptr++) {
		  printf("name: %s, loc: 0x%x\n",
			 symptr->n_un.n_name, symptr->n_value);
			if (symptr->n_type == (N_TEXT|N_EXT) &&
				strncmp(symptr->n_un.n_name,tmp,8)==0) {
