...
main()
{
	...
	if (!ioctl(0, I_LOOK, mname) &&
		...
		if (!svc_reg(transp, RDICTPROG, RDICTVERS, rdictprog_1, 0)) {
			_msgout("unable to register (RDICTPROG, RDICTVERS).");
			exit(1);
		}
		if (pmclose) {
		...
		(void) dup2(i, 2);
		setsid();
		openlog("rdict", LOG_PID, LOG_DAEMON);
#endif
	}
	if (!svc_create(rdictprog_1, RDICTPROG, RDICTVERS, "netpath")) {
		_msgout("unable to create (RDICTPROG, RDICTVERS) for netpath.");
		exit(1);
	}
